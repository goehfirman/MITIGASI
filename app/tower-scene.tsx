...
export default function TowerScene(props:SceneProps){
 ...
 sceneRef.current={scene,group,camera,controls,renderer,platform,grid,editGrid,projections,sim:null,done:false,acc:0,follow:true};controls.addEventListener('start',()=>{if(sceneRef.current)sceneRef.current.follow=false;});let frame=0,last=performance.now(),lastReport=0;
 ...
 useEffect(()=>{const s=sceneRef.current;if(s){s.controls.enabled=props.tool==='orbit'||!!props.run;s.editGrid.visible=props.tool==='add'&&!props.run;s.grid.visible=!s.editGrid.visible;s.editGrid.position.y=props.level*1.5;s.renderer.setPixelRatio(props.low?1:Math.min(devicePixelRatio,1.5));s.renderer.shadowMap.enabled=!props.low;}},[props.tool,props.level,props.run,props.low]);
 ...
 if(props.tool!=='add'||props.run||props.level===0)return;
 const y=props.level*1.5,seen=new Set<string>();
 for(const n of props.design.nodes){
  if(n.y>=y)continue;const key=n.x+','+n.z;
  if(seen.has(key)||props.design.nodes.some(v=>v.y===y&&v.x===n.x&&v.z===n.z))continue;seen.add(key);
  const marker=new T.Mesh(new T.RingGeometry(.09,.21,24),new T.MeshBasicMaterial({color:0xe53935,transparent:true,opacity:.45,side:T.DoubleSide,depthWrite:false}));
  marker.rotation.x=-Math.PI/2;marker.position.set(n.x,y+.012,n.z);marker.userData={x:n.x,z:n.z};s.projections.add(marker);
 }
 },[props.design,props.tool,props.level,props.run]);
